  ...
  try{
    ...
    if(process.env.SENDGRID_API_KEY && process.env.NOTIFY_TO_EMAIL){
      ...
      await fetch("https://api.sendgrid.com/v3/mail/send", {
        ...
        body: JSON.stringify({
          personalizations: [{ to: [{ email: process.env.NOTIFY_TO_EMAIL }] }],
          from: { email: process.env.NOTIFY_TO_EMAIL },
          subject: "Nueva reseña de transporte / New transport review",
          content: [{ type: "text/plain", value: content }]
        })
      });
    }

    // WhatsApp/SMS via Twilio (optional)
    if(process.env.TWILIO_SID && process.env.TWILIO_TOKEN && process.env.TWILIO_FROM && process.env.AUTO_REPLY_WHATSAPP === "true" && phone){
      const body = `¡Gracias por tu reseña! / Thanks for your review! 🙌`;
      const auth = Buffer.from(`${process.env.TWILIO_SID}:${process.env.TWILIO_TOKEN}`).toString("base64");
      const to = phone.startsWith("whatsapp:") ? phone : phone.replace(/\s+/g,"");
